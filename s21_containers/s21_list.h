...
class list {
 ...
 public:
   ...
   */
  list()
      : first_(new node(nullptr, nullptr, value_type{})),
        last_(first_),
        size_(0) {
  ...
  list(list&& other) noexcept : list() { *this = std::move(other); }
 ...
 private:
  ...
  class ListIterator {
   ...
   public:
    ...
    node* getCurrentNode() const noexcept { return current_node_; }
  };
  ...
  class ListConstIterator {
   ...
   public:
    ...
    explicit ListConstIterator(const node* n) : current_node_(n){};
    ...
    node* getCurrentNode() const noexcept {
      return const_cast<node*>(current_node_);
    }
  };
